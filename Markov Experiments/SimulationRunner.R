...
semiMarkov<-function(ntrans=3, ncovs=c(1,1,2), 
  ...
  if (is.na(dist2[i])==FALSE & dist2[i] =="gam") {
    ...
    if (dist[i] == "rps1") {
      print(paste("Running RPS (k=1) model for transition", i))
    ...
    if (!is.na(dist2[i]) && dist2[i] == "rps1") {
      print(paste("Running RPS model (k=1) for transition", i))
      models[[i]] <- flexsurvspline(fmla[[i]], data = datasub[[i]], scale = "hazard", k = 1)
      coeffs[[i]] <- models[[i]]$coefficients[1:ncovs[i]]
      lp[[i]] <- sum(coeffs[[i]] * x[[i]])
      ...
      cumHaz_ext[[i]] <- -log(S_t_ext)
        ...
        ## Royston-Parmar Spline
    if (dist[i] == "rps2") {
      print(paste("Running RPS (k=2) model for transition", i))
    ...
    if (!is.na(dist2[i]) && dist2[i] == "rps2") {
      ...
      cumHaz_ext[[i]] <- -log(S_t_ext)
        ...
        ## Royston-Parmar Spline
    if (dist[i] == "rps3") {
      print(paste("Running RPS (k=3) model for transition", i))
...
doMarkov <- function(msdataContinuous, covEval, dist) {
...
message("Running model... please wait.")
  ...
  results <- applyLifeTableCorrection(results, covEval[1, "startingAge"], covEval[1, "gender"])
  return(results)
}
...
makeGraph <- function(markovOutput) {
  ...
  return(ggplot(simAllcov_long, aes(x = time, y = Probability, fill = State)) +
    geom_area(alpha = 0.7) +
...
runSimulation <- function(simulationName, msdataContinuous) {
...
ethnicityMap <- c("White" = 0, "Black" = 1, "Asian" = 2, "Other" = 3)
    ...
    ethnicity = unname(ethnicityMap["White"]),    
    deprivationIndex = 1,                         
    cvdFH = 0,
    diabetesFH = 1,
    atrialFib = 0,
  ...
  interventionCovariateValues <- unlist(modifyList(as.list(initialCovariateValues), list(
    bmi = 22,  # Ideal HDL >1.6
    glucose = 4.5,  # Ideal fasting glucose 4.5-5.0
    sbp = 140,  # Ideal SBP <120
    dbp = 90,   # Ideal DBP <80
    cholesterol = 7,  # Ideal <5.0
    hdl = 1.0,  # Ideal HDL >1.6
    ldl = 4.0  # Ideal LDL <3.0
  )))
  ...
  dist <- cbind(
    "rps3", "rps3", "rps3", "wei", # Disease-free → T2DM
  ...
  #dist <- cbind("logn", "logn", "logn", "wei", "gom", "gom", "gom", "logn", "wei", "wei", "logn", "wei", "wei")  # Distributions
  ...
  simulationPath <- file.path("simulations", simulationName)

  if (dir.exists(simulationPath)) {
    stop(paste0("A simulation folder already exists at ", simulationPath, ". Choose a different simulationName to avoid overwriting."))
  }
  # Create simulation directory
  dir.create(simulationPath, showWarnings = FALSE, recursive = TRUE)

  # Run Markov simulation
  results <- doMarkov(msdataContinuous, covEval, dist)

  # Save output CSV
  output_csv_path <- file.path(simulationPath, "markov_output.csv")
  data.table::fwrite(results, output_csv_path)

  # Create and save plot
  plot <- makeGraph(results)
  output_plot_path <- file.path(simulationPath, "occupancy_plot.png")
  ggsave(output_plot_path, plot = plot, width = 10, height = 6, dpi = 300)

  # Log covEval to text
  covEval_file <- file.path(simulationPath, "covariate_inputs_by_transition.txt")
  covEvalMatrix <- covEval
  capture.output({
    cat("Covariate values used in each transition:\n\n")
    for (i in 1:nrow(covEvalMatrix)) {
      cat(paste0("Transition ", i, ":\n"))
      print(covEvalMatrix[i, ])
      cat("\n")
    }
  }, file = covEval_file)

  message(paste("Simulation completed and saved to folder:", simulationName))
}